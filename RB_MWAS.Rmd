...
missing <- apply(HILICpos_use, 1, function(x) (sum(x == 0)/ncol(HILICpos_use) <= 0.5))
HILICpos_norm <- HILICpos_use[missing,]; rm(HILICpos_use)
missing <- apply(C18neg_use, 1, function(x) (sum(x == 0)/ncol(C18neg_use) <= 0.5))
...
save(HILICpos_use_t, linkSub_use_hilicpos, linkID_hilicpos, HILICpos_QC, linkSub_QC_hilicpos, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/hilicpos_BatchCorrected_missing50.RData")
save(C18neg_use_t, linkSub_use_c18neg, linkID_c18neg, C18neg_QC, linkSub_QC_c18neg, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/c18neg_BatchCorrected_missing50.RData")
```

### Inspect the quality of sample after imputation

```{r, echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
load(file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/hilicpos_BatchCorrected_missing50.RData")
load(file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/c18neg_BatchCorrected_missing50.RData")
...
class_use_noOUT <-
  ...
  mutate(meduc=cut(meduc, breaks=c(-Inf, 2, 3, Inf), labels=c("<12","12",">=13")))
...
save(HILICpos_norm_t_noOUt, HILICpos_residual, C18neg_norm_t_noOUt, C18neg_residual, class_use_noOUT, linkID_hilicpos, linkID_c18neg, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/DBS_1327_RB_missing50.RData")
```
...
demo <- class_use_noOUT %>%
  ...
  dplyr::select(MRaceShort, birthyear, MotherAge, QUINYOST_birth, parity, GestWeeks, meduc, foreign, Case_Control_Status, Retinoblastoma, sex, AGEFATHER, Feduc)
...
demo <- demo %>%
  mutate(Feduc=cut(Feduc, breaks=c(-Inf, 2, 3, Inf), labels=c("<12","12",">=13"))) %>%
  mutate(AGEFATHER=cut(AGEFATHER, breaks=c(-Inf, 19, 24, 29, 34, Inf), labels=c("<20","20-24","25-29","30-34", ">=35")))
...
table1::table1(~ MRaceShort + birthyear + sex + factor(MotherAge) + factor(QUINYOST_birth) + factor(parity) + factor(GestWeeks) + factor(meduc) + factor(foreign) + factor(Feduc) + factor(AGEFATHER) + Retinoblastoma| Case_Control_Status, data=demo)
table1::table1(~ MRaceShort + birthyear + sex + factor(MotherAge) + factor(QUINYOST_birth) + factor(parity) + factor(GestWeeks) + factor(meduc) + factor(foreign) + factor(Feduc) + factor(AGEFATHER)| Case_Control_Status*Retinoblastoma, data=demo)

summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(MRaceShort), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(birthyear), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(sex), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(MotherAge), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(QUINYOST_birth), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(parity), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(GestWeeks), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(meduc), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(foreign), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(Feduc), data = demo, family = "binomial"))$coefficients
summary(glm(as.factor(demo$Case_Control_Status) ~ as.factor(AGEFATHER), data = demo, family = "binomial"))$coefficients
```
...
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/DBS_1327_RB_missing50.RData")
...
mummichog_name <- paste("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full/HILICpos_mummichog_input","_vip",vip_threshold,"p",0,"_missing50.txt",sep="")
write.table(save.mummichog_PLS_VIP2,file=mummichog_name,sep = "\t",row.names = F,quote = F)
```

```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
save(save.plsresults.allfeatures, save.plsresults.sigfeatures, save.mummichog_PLS_VIP2, save.annotation, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full/HILICpos_MWAS_result_missing50.RData")
```

```{bash, engine.opts='-l'}
mummichog -k /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full -f /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full/HILICpos_mummichog_input_vip1.5p0_missing50.txt -m dpj -c 0.05
```
...
mummichog_name <- paste("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full/C18neg_mummichog_input","_vip",vip_threshold,"p",0,"_missing50.txt",sep="")
write.table(save.mummichog_PLS_VIP2,file=mummichog_name,sep = "\t",row.names = F,quote = F)
```

```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
save(save.plsresults.allfeatures, save.plsresults.sigfeatures, save.mummichog_PLS_VIP2, save.annotation, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full/C18neg_MWAS_result_missing50.RData")
```

```{bash, engine.opts='-l'}
mummichog -k /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full -f /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full/C18neg_mummichog_input_vip1.5p0_missing50.txt -m negative -c 0.05
```
...
# Sensitivity analysis - Agedxmonth

```{r , echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
# Investigate the relationship between DxAge and type of RB
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/DBS_1327_RB.RData")
class_use_noOUT$Case_Control_Status <- factor(class_use_noOUT$Case_Control_Status, levels=c(0,1),labels=c("Control","Case"))
class_use_noOUT <-
  class_use_noOUT %>%
  dplyr::mutate(type = case_when(Unilateral == 1 ~ "Unilateral", 
                                 Bilateral == 1 ~ "Bilateral", 
                                 Unilateral == 0 & Bilateral == 0 ~ "Control"))
class_use_noOUT %>%
  filter(type != "Control") %>%
  group_by(type) %>%
  summarise(n = n(), mean = mean(AgeDxMonths), max = max(AgeDxMonths), min = min(AgeDxMonths), sd = sd(AgeDxMonths))

class_use_noOUT %>%
  filter(type != "Control") %>%
  filter(AgeDxMonths <= 18) %>%
  group_by(type) %>%
  summarise(n = n())

class_use_noOUT %>%
  filter(type != "Control") %>%
  ggplot(aes(x=AgeDxMonths, fill=type)) +
...
```{r , echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
# Decide to calculate the correlation of coefficients derived from linear regression between bi and uni cases
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Unilateral/C18neg_MWAS_result.RData")
Uni_coef <- save.plsresults.allfeatures;rm(save.annotation, save.mummichog_PLS_VIP2, save.plsresults.allfeatures, save.plsresults.sigfeatures)
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Bilateral/C18neg_MWAS_result.RData")
Bi_coef <- save.plsresults.allfeatures;rm(save.annotation, save.mummichog_PLS_VIP2, save.plsresults.allfeatures, save.plsresults.sigfeatures)
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full/C18neg_MWAS_result.RData")
full_coef <- save.plsresults.allfeatures;rm(save.annotation, save.mummichog_PLS_VIP2, save.plsresults.allfeatures, save.plsresults.sigfeatures)

cor(Uni_coef$coef, full_coef$coef, method = "pearson")
cor(Bi_coef$coef, full_coef$coef, method = "pearson")
cor(Uni_coef$coef, Bi_coef$coef, method = "pearson")
```

Restricted to AgeDxmonths < 18 and stratified by type

```{r , echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/DBS_1327_RB.RData")
class_use_noOUT$Case_Control_Status <- factor(class_use_noOUT$Case_Control_Status, levels=c(0,1),labels=c("Control","Case"))

# Restrict to AgeDxMonths < 18
class_use_noOUT <- class_use_noOUT %>%
  dplyr::filter(is.nan(AgeDxMonths) | AgeDxMonths <= 18)
table(class_use_noOUT$Case_Control_Status)

# Statified by type
# {
#   type = "Unilateral"
#   class_use_noOUT <- class_use_noOUT[which(!is.na(class_use_noOUT$Unilateral)),]
# }
table(class_use_noOUT$Case_Control_Status)
HILICpos_norm_t_noOUt <- HILICpos_norm_t_noOUt[class_use_noOUT$hilicpos,]
HILICpos_residual <- HILICpos_residual[class_use_noOUT$hilicpos,]
...
feature_pvalue <- as.data.frame(feature_pvalue)
feature_pvalue$adjust_pvalue <- p.adjust(feature_pvalue$pvalue, method="BH")
rownames(feature_pvalue) <- rownames(linkid)

print(paste("Logistic regression:", sum(feature_pvalue$pvalue <= 0.05), "features are significant (raw_p<=0.05)", sep = " "))
print(paste("Logistic regression:", sum(feature_pvalue$adjust_pvalue <= 0.2), "features are significant (adj_p<=0.2)", sep = " "))
print(paste("Logistic regression:", sum(feature_pvalue$adjust_pvalue <= 0.05), "features are significant (adj_p<=0.05)", sep = " "))

save(feature_pvalue, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/dx18_full_hilicpos.RData")
```
...
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/dx18_full_hilicpos.RData")
full_coef_dx18 <- feature_pvalue; rm(feature_pvalue)
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/dx18_uni_hilicpos.RData")
Uni_coef_dx18 <- feature_pvalue; rm(feature_pvalue)
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/dx18_bi_hilicpos.RData")
Bi_coef_dx18 <- feature_pvalue; rm(feature_pvalue)

cor(full_coef_dx18$coef, full_coef$coef, method = "pearson")
cor(Bi_coef_dx18$coef, Bi_coef$coef, method = "pearson")
cor(Uni_coef_dx18$coef, Uni_coef$coef, method = "pearson")

cor(Uni_coef_dx18$coef, full_coef_dx18$coef, method = "pearson")
cor(Bi_coef_dx18$coef, full_coef_dx18$coef, method = "pearson")
cor(Uni_coef_dx18$coef, Bi_coef_dx18$coef, method = "pearson")
```

# Sensitivity analysis - Gestational diabetes

Conduct MWAS on gestational diabetes and compare it w/ RB signals?

Only 8 participants had CPgestDiab == 1, so not able to conduct this analysis

# Sensitivity analysis - Diet biomarker

https://academic.oup.com/cdn/article/4/10/nzaa144/5900522?login=true

In Figure 1 they listed 5 or 6 biomarkers for healthy and unhealthy dietary patterns. They go on to say:

“The correlations between dietary scores and circulating metabolites are generally modest (r ∼0.2–0.4), though robust correlations exist between certain foods and certain serum metabolites that are consistent across cohorts irrespective of fasting status, age, prepregnancy BMI, ethnicity, and/or region…….the average intake of vegetables and/or fruit was associated with serum hippuric acid (a major metabolite of flavonoids prevalent in fruits and vegetables) (47), whereas self-reported consumption of seafood, meat, and eggs was also correlated with serum TMAO (present in free form in fish and animal flesh, and also generated from the actions of host and gut microflora cometabolism of carnitine from intake of meat or eggs) (48, 49). Additionally, intake of nuts/legumes was associated with circulating concentrations of tryptophan betaine (which accumulates in the seeds of most Erythrina species) (50), and was previously reported to be associated with peanut intake (51). In future investigations of maternal diet and child health outcomes these serum metabolites can be combined to constitute a “metabolic signature” reflecting a healthy diet, and generalizable to a multiethnic population.”

```{r , echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
load(file = "D:/Dropbox/Metabolomics_Retinoblastoma/hilicpos/HILIC_verified_features.RData")
load(file = "D:/Dropbox/Metabolomics_Retinoblastoma/c18neg/C18_verified_features.RData")

# We only have Hippurate in our data, Hippurate associates with intake of vegetables/fruit/whole grains etc. And it associates with higher Shannon diversity, and lower risk of MetS 

# https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5651863/

ID <- c("HMDB00714")
HILIC_verify <- HILIC_verify %>%
  filter(HMDBID %in% ID)
C18_verify <- C18_verify %>%
  filter(HMDBID %in% ID)
```

```{r , echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
# Investigate the relationship between Hippurate and RB
load("D:/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/DBS_1327_RB.RData")
# load(file = "D:/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/hilicpos_BatchCorrected.RData")
# HILICpos_use_t <- HILICpos_use_t[rownames(HILICpos_norm_t_noOUt),]
class_use_noOUT$Case_Control_Status <- factor(class_use_noOUT$Case_Control_Status, levels=c(0,1),labels=c("Control","Case"))

hilic_hippurate <- HILICpos_norm_t_noOUt[,rownames(HILIC_verify)]
c18_hippurate <- C18neg_norm_t_noOUt[,rownames(C18_verify)]

# hilic_hippurate <- HILICpos_residual[,rownames(HILIC_verify)]
# c18_hippurate <- C18neg_residual[,rownames(C18_verify)]

cor(hilic_hippurate, c18_hippurate)
...
class_use_noOUT <-
  ...
  dplyr::mutate(type = case_when(Unilateral == 1 ~ "Unilateral", 
                                 Bilateral == 1 ~ "Bilateral", 
                                 Unilateral == 0 & Bilateral == 0 ~ "Control")) %>%
  dplyr::mutate(type = factor(type, levels=c("Control","Unilateral","Bilateral"),labels=c("Control","Unilateral","Bilateral"))) %>%
  dplyr::mutate(hilic_hip = hilic_hippurate) %>%
  dplyr::mutate(c18_hip = c18_hippurate)

class_use_noOUT %>%
  group_by(type) %>%
  summarise(n = n(), mean = mean(hilic_hip), max = max(hilic_hip), min = min(hilic_hip), sd = sd(hilic_hip))
class_use_noOUT %>%
  group_by(type) %>%
  summarise(n = n(), mean = mean(c18_hip), max = max(c18_hip), min = min(c18_hip), sd = sd(c18_hip))

class_use_noOUT %>%
    ggplot(aes(x=type, y=hilic_hip, fill=type)) +
    geom_boxplot() +
    viridis::scale_fill_viridis(discrete = TRUE, alpha=0.6) +
    geom_jitter(color="black", size=0.4, alpha=0.9) +
    hrbrthemes::theme_ipsum() +
...
class_use_noOUT %>%
    ggplot(aes(x=type, y=c18_hip, fill=type)) +
    geom_boxplot() +
    viridis::scale_fill_viridis(discrete = TRUE, alpha=0.6) +
    geom_jitter(color="black", size=0.4, alpha=0.9) +
    hrbrthemes::theme_ipsum() +
...
# Sensitivity analysis - Parternal age
...
table(class_use_noOUT$foreign)
table(class_use_noOUT$sex)
table(class_use_noOUT$Feduc)
table(class_use_noOUT$AGEFATHER)

removed <- class_use_noOUT$Barcode[which(is.na(class_use_noOUT$MRaceShort)|is.na(class_use_noOUT$birthyear)|is.na(class_use_noOUT$MotherAge)|is.na(class_use_noOUT$parity)|is.na(class_use_noOUT$QUINYOST_birth)|is.na(class_use_noOUT$GestWeeks)|is.na(class_use_noOUT$foreign)|is.na(class_use_noOUT$sex)|is.na(class_use_noOUT$Feduc)|is.na(class_use_noOUT$AGEFATHER))]
...
fit_feature <- lm(data = HILICpos_feature_w_cov, as.matrix(HILICpos_feature_w_cov[,718:ncol(HILICpos_feature_w_cov)]) ~ as.factor(MRaceShort) + as.factor(birthyear) + as.factor(MotherAge) + as.factor(QUINYOST_birth) + as.factor(parity) + as.factor(GestWeeks) + as.factor(foreign) + as.factor(sex) + as.factor(Feduc) + as.factor(AGEFATHER), na.action = na.exclude)
...
fit_feature <- lm(data = C18neg_feature_w_cov, as.matrix(C18neg_feature_w_cov[,718:ncol(C18neg_feature_w_cov)]) ~ as.factor(MRaceShort) + as.factor(birthyear) + as.factor(MotherAge) + as.factor(QUINYOST_birth) + as.factor(parity) + as.factor(GestWeeks) + as.factor(foreign) + as.factor(sex) + as.factor(Feduc) + as.factor(AGEFATHER), na.action = na.exclude)
residual_feature <- as.matrix(residuals(fit_feature),nrow = dim(C18neg_feature_w_cov)[1])
save_residual <- as.data.frame(residual_feature)
rownames(save_residual) <- rownames(C18neg_norm_t_noOUt)
C18neg_residual <- save_residual; rm(save_residual, residual_feature, fit_feature)
sum(is.na(C18neg_residual))
save(HILICpos_norm_t_noOUt, HILICpos_residual, C18neg_norm_t_noOUt, C18neg_residual, class_use_noOUT, linkID_hilicpos, linkID_c18neg, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/DBS_1088_RB_paternal.RData")
```
...
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/DBS_1088_RB_paternal.RData")
class_use_noOUT$Case_Control_Status <- factor(class_use_noOUT$Case_Control_Status, levels=c(0,1),labels=c("Control","Case"))

group <- "Case_Control_Status"

# restricted to bilateral
class_use_noOUT <- class_use_noOUT[which(!is.na(class_use_noOUT$Bilateral)),]
table(class_use_noOUT$Case_Control_Status)
HILICpos_norm_t_noOUt <- HILICpos_norm_t_noOUt[class_use_noOUT$hilicpos,]
HILICpos_residual <- HILICpos_residual[class_use_noOUT$hilicpos,]
...
lm_func <- function(x) glm(class_use_noOUT[[group]] ~ x + as.factor(MRaceShort) + as.factor(birthyear) + as.factor(MotherAge) + as.factor(QUINYOST_birth) + as.factor(parity) + as.factor(GestWeeks) + as.factor(foreign) + as.factor(sex) + as.factor(Feduc) + as.factor(AGEFATHER), data = feature_w_cov, family = "binomial")
...
# Effective number of testing (95% and 99%)
pca.datExpr = mixOmics::pca(X, ncomp = 800, center = TRUE, scale = TRUE)
ent <- sum(pca.datExpr$explained_variance)
ent <- summary(pca.datExpr)$cum.var
ent95 <- 0.05/sum(ent<0.95)
ent99 <- 0.05/sum(ent<0.99)
...
mummichog_name <- paste("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/HILICpos_mummichog_input","_vip",vip_threshold,"p",0,"_paternal.txt",sep="")
write.table(save.mummichog_PLS_VIP2,file=mummichog_name,sep = "\t",row.names = F,quote = F)
```

```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
save(save.plsresults.allfeatures, save.plsresults.sigfeatures, save.mummichog_PLS_VIP2, save.annotation, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/HILICpos_MWAS_result_paternal.RData")
```

```{bash, engine.opts='-l'}
mummichog -k /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity -f /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/HILICpos_mummichog_input_vip1.5p0_paternal.txt -m dpj -c 0.05
```
...
table(class_use_noOUT$Case_Control_Status)
C18neg_norm_t_noOUt <- C18neg_norm_t_noOUt[class_use_noOUT$c18neg,]
C18neg_residual <- C18neg_residual[class_use_noOUT$c18neg,]
...
mummichog_name <- paste("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/C18neg_mummichog_input","_vip",vip_threshold,"p",0,"_paternal.txt",sep="")
write.table(save.mummichog_PLS_VIP2,file=mummichog_name,sep = "\t",row.names = F,quote = F)
```

```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
save(save.plsresults.allfeatures, save.plsresults.sigfeatures, save.mummichog_PLS_VIP2, save.annotation, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/C18neg_MWAS_result_paternal.RData")
```

```{bash, engine.opts='-l'}
mummichog -k /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity -f /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/C18neg_mummichog_input_vip1.5p0_paternal.txt -m negative -c 0.05
```
...
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
HILIC <- final_table(mwas_data = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/HILICpos_MWAS_result.RData", wd_pathway = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/hilicpos/tables/")

C18 <- final_table(mwas_data = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/C18neg_MWAS_result.RData", wd_pathway = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/c18neg/tables/")

write.table(HILIC, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/HILIC_final_table.txt", sep = "\t", row.names = F,quote = F)
write.table(C18, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Sensitivity/C18_final_table.txt", sep = "\t", row.names = F,quote = F)
```

# Note

Find a way to organize the final feature table

```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
a <- xlsx::read.xlsx(file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Manuscript/Tables_v1.xlsx", sheetName = "Sheet8")
a$Name <- sub(".*C[0-9]*, ", "", a$CompoundName)
a$ID <- substr(a$CompoundName, 13, 18)
write.table(a, file = "/Users/qiyan/Desktop/a.txt", sep = "\t", row.names = F, col.names = T, quote = F)
```