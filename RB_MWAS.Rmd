...
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
load(file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full/HILICpos_MWAS_result.RData")
PIUmet <- save.plsresults.sigfeatures %>%
  dplyr::filter(adjust_pvalue < 0.05) %>%
  dplyr::select(mz, pvalue) %>%
  dplyr::mutate(pvalue = -log10(pvalue)) %>%
  dplyr::mutate(mode = "positive") %>%
  dplyr::select(mz, mode, pvalue)
load(file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Output/Full/C18neg_MWAS_result.RData")
PIUmet_2 <- save.plsresults.sigfeatures %>%
  dplyr::filter(adjust_pvalue < 0.05) %>%
  dplyr::select(mz, pvalue) %>%
  dplyr::mutate(pvalue = -log10(pvalue)) %>%
  dplyr::mutate(mode = "negative") %>%
  dplyr::select(mz, mode, pvalue)
  
PIUmet <- rbind(PIUmet, PIUmet_2); rm(PIUmet_2)
write.table(PIUmet, file = "/Users/qiyan/Desktop/test.txt", sep = "\t", row.names = F, col.names = F, quote = F)
```
...
volcano <- save.plsresults.sigfeatures[,c(2,3,8,15)]

volcano$group <- ifelse(volcano$pvalue<=-log10(0.05)&volcano$foldchange>=1,3,
                        ifelse(volcano$pvalue<=0.05&volcano$foldchange<=-1,2,
                               ifelse(volcano$pvalue<=0.05&abs(volcano$foldchange)<=1,1,0)))
volcano$size <- ifelse(volcano$group == 0,0,1)
ggplot2::ggplot(volcano,aes(x=foldchange,y=vip)) +
  geom_point(aes(colour=cut(group, c(-Inf,0,1,2,Inf))),show.legend = FALSE, cex=2.5) + 
  xlim(-3.8,3.8) +
  scale_fill_hue(c=20, l=20) + 
  scale_color_manual(values = c("#999999","goldenrod3","springgreen3","firebrick1")) + 
  scale_size_continuous(range = c(1,3)) +
  geom_hline(aes(yintercept = 2),color = "red",size = 0.5,linetype = "dashed") +
  geom_vline(aes(xintercept = 1),color = "red",size = 0.5,linetype = "dashed") +
  geom_vline(aes(xintercept = -1),color = "red",size = 0.5,linetype = "dashed") +
...
dataA <- read.table(file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/c18neg/ComBat_mzcalibrated_untargeted_mediansummarized_featuretable.txt", header = T, sep = "\t")
dataA <- dataA[,-c(3:10)]
...
#OR
#dataA<-read.table("/Users/data.txt",sep="\t",header=TRUE)

#output location
outloc<-"/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/c18neg/Annotation/"
...
num_nodes<-8   #number of cores to be used; 2 is recommended for desktop computers due to high memory consumption

db_name="T3DB" #other options: HMDB,Custom,KEGG, LipidMaps, T3DB
