...
class_use_noOUT <-
  ...
  mutate(GestWeeks=cut(GestWeeks, breaks=c(-Inf, 27, 31, 36, Inf), labels=c("extreme_preterm","very_preterm","preterm","term"))) %>%
...
fit_feature <- lm(data = HILICpos_feature_w_cov, as.matrix(HILICpos_feature_w_cov[,718:ncol(HILICpos_feature_w_cov)]) ~ as.factor(MRaceShort) + as.factor(birthyear) + as.factor(MotherAge) + as.factor(QUINYOST_birth) + as.factor(parity) + as.factor(foreign), na.action = na.exclude)
...
fit_feature <- lm(data = C18neg_feature_w_cov, as.matrix(C18neg_feature_w_cov[,718:ncol(C18neg_feature_w_cov)]) ~ as.factor(MRaceShort) + as.factor(birthyear) + as.factor(MotherAge) + as.factor(QUINYOST_birth) + as.factor(parity) + as.factor(foreign), na.action = na.exclude)
...
save(HILICpos_norm_t_noOUt, HILICpos_residual, C18neg_norm_t_noOUt, C18neg_residual, class_use_noOUT, linkID_hilicpos, linkID_c18neg, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Input/DBS_1327_PTB.RData")
```
...
load(file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_RB/Input/hilicpos_BatchCorrected.RData")
load("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Input/DBS_1327_PTB.RData")
class_use_noOUT <- class_use_noOUT[which(class_use_noOUT$Case_Control_Status==0),]
...
table1::table1(~ MRaceShort + birthyear + sex + factor(MotherAge) + factor(QUINYOST_birth) + factor(parity) + factor(meduc) + factor(foreign) + factor(Feduc) + factor(AGEFATHER) | factor(GestWeeks), data=demo)
```
...
class_use_noOUT <- class_use_noOUT[which(class_use_noOUT$Case_Control_Status==0),]
table(class_use_noOUT$GestWeeks)
class_use_noOUT$GestWeeks <- ifelse(class_use_noOUT$GestWeeks=="term", "term", "preterm")
table(class_use_noOUT$GestWeeks)
...
fs_mode <- "lm" ## "lm", "en", "spls", "plsda", or "plslm"
...
fit_feature <- lm(data = feature_w_cov, as.matrix(feature_w_cov[,718:ncol(feature_w_cov)]) ~ class_use_noOUT[[group]] + as.factor(MRaceShort) + as.factor(birthyear) + as.factor(MotherAge) + as.factor(QUINYOST_birth) + as.factor(parity) + as.factor(foreign), na.action = na.exclude)
...
for(i in 1:ncol(X_fc)){
  case.temp <- X_fc[which(as.character(Y) == "preterm"),i]
  case <- mean(case.temp[!(abs(case.temp - median(case.temp)) > 2*sd(case.temp))])
  control.temp <- X_fc[which(as.character(Y) == "term"),i]
  control <- mean(control.temp[!(abs(control.temp - median(control.temp)) > 2*sd(control.temp))])
  fc.empty[i,1] <- case
  fc.empty[i,2] <- control
}
colnames(fc.empty) <- c("preterm","term")
fc.empty$foldchange = fc.empty$preterm-fc.empty$term
fc.updown <- fc.empty
...
vip.for.selection$group[vip.for.selection$adjust_pvalue >= 0.05] <- 0
vip.for.selection$group[vip.for.selection$adjust_pvalue < 0.05 & vip.for.selection$coef <= 0] <- 1 ## down
vip.for.selection$group[vip.for.selection$adjust_pvalue < 0.05 & vip.for.selection$coef > 0] <- 2  ## up

ggplot2::ggplot(vip.for.selection,aes(x=mz,y=-log10(adjust_pvalue))) +
  ...
  geom_hline(aes(yintercept = 1.30103),color = "red",size = 0.5,linetype = "dashed") +
  scale_x_continuous(breaks = pretty(vip.for.selection$mz, n = 10))+
  scale_y_continuous(breaks = pretty(-log(vip.for.selection$adjust_pvalue), n = 10))+
  ...
  labs(x="M/Z",y="-Log10(FDR)") +
  ggtitle("Type 1 manhattan plot (FDR vs mz)")

ggplot2::ggplot(vip.for.selection,aes(x=time,y=-log10(adjust_pvalue))) +
  geom_point(aes(colour = cut(group, c(-Inf,0,1,2,Inf))),size=1,show.legend = FALSE) + 
  scale_fill_hue(c=20, l=20) + 
  scale_color_manual(values = c("#999999","springgreen3","firebrick1")) +
  geom_hline(aes(yintercept = 1.30103),color = "red",size = 0.5,linetype = "dashed") +
  ...
  theme(axis.text=element_text(face="bold"),
        ...
        axis.title.x = element_text(size=12, face="bold"),
        axis.title.y = element_text(size=12, face="bold"))+
  labs(x="Retention time",y="-Log10(FDR)") +
  ggtitle("Type 1 manhattan plot (FDR vs Time)")

volcano <- vip.for.selection[,c(2,3,9,15)]
volcano$group <- ifelse(volcano$adjust_pvalue<=0.05&volcano$foldchange>=0.58,3,
                        ifelse(volcano$adjust_pvalue<=0.05&volcano$foldchange<=-0.58,2,
                               ifelse(volcano$adjust_pvalue<=0.05&abs(volcano$foldchange)<=0.58,1,0)))
volcano$size <- ifelse(volcano$group == 0,0,1)

ggplot2::ggplot(volcano,aes(x=foldchange,y=-log10(adjust_pvalue))) +
...
if(fs_mode == "lm"){
  good_feats_ordered <- subset(vip.for.selection, adjust_pvalue < 0.05)
...
```{r , echo=TRUE, eval=TRUE, message=FALSE, warning=FALSE}
Y = factor(Y, levels=c("term","preterm"),labels=c("Control","Case"))
good_feats_ordered.name <- {}
for (i in 1:nrow(good_feats_ordered)) {
  good_feats_ordered.name <- c(good_feats_ordered.name,which(colnames(X)==row.names(good_feats_ordered[i,])))
}
...
save.mummichog_PLS_VIP2 <- save.plsresults.allfeatures[,c(2,3,7,9)]
...
save.mummichog_PLS_VIP2$`p-value`[save.mummichog_PLS_VIP2$adjust_pvalue<0.05] <- 0.04
save.mummichog_PLS_VIP2 <- save.mummichog_PLS_VIP2[order(save.mummichog_PLS_VIP2$`p-value`,save.mummichog_PLS_VIP2$adjust_pvalue),]
save.mummichog_PLS_VIP2 <- save.mummichog_PLS_VIP2[,c(1,2,5,3)]

mummichog_name <- paste("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/HILICpos_mummichog_input_lm.txt",sep="")
write.table(save.mummichog_PLS_VIP2,file=mummichog_name,sep = "\t",row.names = F,quote = F)
```

```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
save(save.plsresults.allfeatures, save.plsresults.sigfeatures, save.mummichog_PLS_VIP2, save.annotation, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/HILICpos_MWAS_result.RData")
```

```{bash, engine.opts='-l'}
mummichog -k /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/ -f /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/HILICpos_mummichog_input_lm.txt -m dpj -c 0.05
```
...
# tolerance = 5
# 
# C18_library <- read.xlsx(file = "/Users/qiyan/Dropbox/PEG/PD_Pest_MultiOmics/Raw_Data/InHouse_Library_Emory/Combined_comfirmed_metabolite_list_08032020.xlsx", 1, header = T) %>%
#   distinct(KEGGID, .keep_all= TRUE)
# 
...
mummichog_name <- paste("/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/C18neg_mummichog_input_lm.txt",sep="")
write.table(save.mummichog_PLS_VIP2,file=mummichog_name,sep = "\t",row.names = F,quote = F)
```

```{r , echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
save(save.plsresults.allfeatures, save.plsresults.sigfeatures, save.mummichog_PLS_VIP2, save.annotation, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/C18neg_MWAS_result.RData")
```

```{bash, engine.opts='-l'}
mummichog -k /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/ -f /Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/C18neg_mummichog_input_lm.txt -m negative -c 0.05
```
...
```{r, echo=TRUE, eval=FALSE, message=FALSE, warning=FALSE}
HILIC <- final_table(mwas_data = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/HILICpos_MWAS_result.RData", wd_pathway = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/hilicpos/tables/")

C18 <- final_table(mwas_data = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/C18neg_MWAS_result.RData", wd_pathway = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/c18neg/tables/")

write.table(HILIC, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/HILIC_final_table.txt", sep = "\t", row.names = F,quote = F)
write.table(C18, file = "/Users/qiyan/Dropbox/Metabolomics_Retinoblastoma/MWAS_PTB/Output/C18_final_table.txt", sep = "\t", row.names = F,quote = F)
```